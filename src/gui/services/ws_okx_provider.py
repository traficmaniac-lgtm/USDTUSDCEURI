"""WebSocket provider for OKX quotes."""
...
class OkxWsProvider(WsProviderBase):
    """Threaded WebSocket worker for OKX ticker updates."""
    ...
    def run(self) -> None:
        ...
        def on_message(_ws: websocket.WebSocketApp, message: str) -> None:
            ...
            try:
                ...
                return
            timestamp = self._format_timestamp(item.get("ts"))
            self._emit_quote(
                bid=bid_value,
                ask=ask_value,
                last=last_value,
